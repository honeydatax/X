...
int main(void) {
  ...
  XSetWMProtocols(d, w, &WM_DELETE_WINDOW, 1);  
   ...
   while (1) {
      XNextEvent(d, &e);
      if (e.type == Expose) {
				XFillRectangle(d, w, DefaultGC(d, s),15,30,500,5);
				XFillRectangle(d, w, DefaultGC(d, s),8,30,10,32);
				XFillRectangle(d, w, DefaultGC(d, s),500,30,18,32);
				XFillRectangle(d, w, DefaultGC(d, s),15,60,500,5);
      }
          if (e.type == KeyPress)
    {
      char buf[128] = {0};
